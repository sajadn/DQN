...
class DQNBaseModel(modelBase):
	...
	def executeStep(self, input_val, output, target_action_mask, prio_weights = np.ones(params.mini_batch_size)):
	...
	def defineLossAndTrainer(self):
		with tf.name_scope("loss"):
			...
			if(params.regularization==1):
				for weightRegul in self.weights[0::2]:
					self.loss += (1/2)*params.regularization_factor * tf.reduce_sum(tf.square(weightRegul))
